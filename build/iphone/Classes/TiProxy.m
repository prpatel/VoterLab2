...
//Rare exceptions to indicate a bug in the _voterlab code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_voterlab = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_voterlab]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.voterlab.module"
